...
public extension CGRect {
    ...
    func cropped(to size: CGSize, centering: Bool = true) -> CGRect {
        if centering {
            let horizontalDiff = self.width - size.width
            let verticalDiff = self.height - size.height
            
            let newOrigin = CGPoint(x: self.origin.x + (horizontalDiff / 2.0),
                                    y: self.origin.y + (verticalDiff / 2.0))
...
public extension UIImage {
    ...
    var invertTransform: CGAffineTransform {
        CGAffineTransform(scaleX: 1, y: -1).translatedBy(x: 0, y: -self.height)
    }
