...
extension VNRequest {
    func queueFor(image: UIImage, completion: @escaping ([Any]?) -> Void) {
...
extension UIImage {
    func detectRectangles(completion: @escaping ([VNRectangleObservation]) -> Void) {
    ...
    func detectBarcodes(types symbologies: [VNBarcodeSymbology] = [.QR],
...
barcodeTestImage.detectBarcodes { barcodes in
    for code in barcodes {
        print("Barcode data: \(code.payloadStringValue ?? "None")")
    }
}
